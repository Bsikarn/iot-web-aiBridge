...
- **Upstash Redis Task Queue & Polling Architecture (`/api/ask` & `/api/status`)** — Powered by `@upstash/redis`, `/api/ask` initializes tasks in Redis, returns `HTTP 202 Accepted` with a unique `task_id`, and delegates background processing via a single `@vercel/functions` `waitUntil()` worker. Hardware and web clients poll `GET /api/status?task_id=<id>` for completion, eliminating HTTP request timeouts during long-reasoning AI queries with automatic 15-minute Redis TTL expiry.
- **Multi-Image Processing & Deduplication (Up to 4 Unique Images)** — `/api/ask` extracts unique sequential image uploads (`image_1` through `image_4`, `images`, or `image_urls`) using a `Set` deduplicator. Uploads each unique image to Discord CDN exactly once and constructs a sequential multimodal payload for OpenRouter LLMs to synthesize multi-page problem snapshots without duplicate uploads or double-billing.
