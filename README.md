...
- **Custom Model Slot Names & Dual-Model Group Configuration** — Model slots (1 to 10) support custom display labels (e.g. "Sol+Sonnet", "Claude-Ckt") returned directly in `GET /api/settings` for hardware menu rendering, alongside an optional secondary OpenRouter Model ID for parallel execution.
- **Full-Resolution 3-Column Grid Stitching (1–12 Images)** — `/api/ask` extracts up to 12 sequential image uploads (`image_1` to `image_12` or `images`) and stitches them using `canvas` into a single composite grid image (3 columns x N rows) at **100% full native resolution** with 2px dark separation borders and zero image text overlay.
- **Upstash Redis Asynchronous Polling Architecture (`/api/ask` & `/api/status`)** — Powered by `@upstash/redis`, `/api/ask` initializes tasks in Redis, returns `HTTP 202 Accepted` with a unique `task_id` in <1s, and updates step-by-step progress tracking (`received` -> `grid_done` -> `ai_processing` -> `completed` / `failed`) for client polling `GET /api/status?task_id=<id>`.
- **Parallel Dual-LLM Execution & Sequential Canvas Pagination** — Supports calling both primary and secondary models concurrently via `Promise.all([callAI(m1), callAI(m2)])` with Ephemeral Prompt Caching (`cache_control: ephemeral`) and uncapped output token capacity, rendering solutions sequentially onto monochrome 250x122 Base64 PNG E-Paper pages.
