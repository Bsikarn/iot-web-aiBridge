...
- **Intl.Segmenter Thai Word-Segmentation Preprocessor** — Leverages native JavaScript `Intl.Segmenter` (`granularity: 'word'`) to automatically segment Thai text and insert zero-width spaces (`\u200b`). Combined with CSS `word-break: break-word; overflow-wrap: break-word; white-space: pre-wrap;`, Thai sentences wrap cleanly line-by-line at dictionary word boundaries without mid-word splits or 250px container overflow. Includes custom Markdown parser (`convertMarkdownToHtml`) converting `**text**` into clean bold HTML tags (`<strong>text</strong>`).
