...
- **Anthropic & OpenRouter Prompt Caching (`cache_control: ephemeral`)** — Integrates native Anthropic and OpenRouter Prompt Caching on the System Instructions and Knowledge Base (`<knowledge_base>`) content blocks. Reduces input token costs by up to **90%** and significantly lowers response latency during repeated queries.
- **Maximized Vertical Content Area (Header Removal)** — Completely removed top divider lines, headers, and badge titles from the 250x122px page generation template (`padding: 2px 4px !important`). Answer content and math formulas start immediately from the very top of the page, utilizing 100% of the available 122px vertical height.
