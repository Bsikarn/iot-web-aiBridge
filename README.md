...
- **Thai Word Wrap & Clean Markdown Rendering** — Applied `word-break: break-word; overflow-wrap: break-word; white-space: pre-wrap;` to strictly prevent Thai script line overflow beyond the 250px boundary. Included custom Markdown parser (`convertMarkdownToHtml`) converting `**text**` into clean bold HTML tags (`<strong>text</strong>`) without raw `**` text artifacts.
