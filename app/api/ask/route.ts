  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (imagesArray.length > 0) {
        imageFiles = imagesArray.slice(0, 4);
      } else if (individualImages.length > 0) {
        imageFiles = individualImages.slice(0, 4);
      }
    ...
    if (mode === 'reuse') {
      ...
      if (imageFiles.length === 0 && jsonImageUrls.length === 0) {
      ...
      if (imageFiles.length > 0) {
        for (let i = 0; i < imageFiles.length; i++) {
          ...
          const base64DataUrl = `data:${mimeType};base64,${Buffer.from(arrayBuffer).toString("base64")}`;

          // Upload each image to Discord Webhook and get Discord CDN URL
          try {
            const uploadedUrl = await uploadToDiscordWebhook(
              arrayBuffer,
              file.name || `snap-${Date.now()}-${i + 1}.jpg`,
              mimeType
            );
...
3. UNIFORM TYPOGRAPHY: Use standard regular-weight plain text and markdown formatting so all text renders cleanly at a uniform weight on the monochrome E-Ink hardware screen. Only use LaTeX math wrappers ($...$ or $$...$$) for complex multi-term equations.`;
    ...
    // Append sequential image blocks
